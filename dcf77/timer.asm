...
PRPUH1		MOV A,R4				; restore count of previous four samples
			...
			JC PRPUH2				; previous was low, beginning of new pulse (second)
			INC @R0					; previous was high, pulse still unfinished, increment length
			RET
PRPUH2		MOV R0,#CURR_STAT		; get address of current state variable
...
PRPUL1		INC @R0					; increment length variable address
			...
			SUBI(2)					; number of ones in previous 100ms period less then 2?
			JC PRPUL4				; yes, still no new pulse detected
			...
			JNC PRPUL2				; yes, it is long pulse e.g. PULSE_ONE
			MOV A,#PULSE_ZERO		; no, PULSE_ZERO
			JMP PRPUL3
PRPUL2		MOV A,#PULSE_ONE		; PULSE_ONE
PRPUL3		MOV R0,#CURR_STAT		; get address of current state variable
			ORL A,@R0				; combine values
			XCH A,@R0				; exchange values (set CURR_STAT)
			RET
PRPUL4		MOV R0,#PULSE_LEN		; get pulse length variable address to R0
			MOV A,@R0				; get pulse length
