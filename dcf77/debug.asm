...
LOGINIT		MOV R1,#CTRUART			; 8251A control register address
			CLR A					; clear A
			MOVX @R1,A				; 00H to control regiter
			MOVX @R1,A				; 00H to control regiter
			MOVX @R1,A				; 3x 00H to control regiter
			MOV A,#40H				; reset 40h
			MOVX @R1,A				; register write
			...
			RET
			;
			; log with registers preserved - R1 is changed though
LOGACC		ANL P2,#~IOM8155		; select 8155 RAM(deny UART, RIOT)
			MOV R1,#BACK_A			; backup RAM address
			MOVX @R1,A				; backup A
...
_TXBY4		MOV R2,A				; move to R2
	 		MOV R1,#CTRUART			; 8251A control/status reg
_TXC1		MOVX A,@R1				; status register to A
			JB0 _TXC2				; bit 0 TxRDY - buffer empty - proceed sending
			JMP _TXC1				; wait more
_TXC2		DEC R1					; 8251A data reg
...
TXBY4		MOV R2,A				; move to R2
	 		MOV R1,#CTRUART			; 8251A control/status reg
TXC1		MOVX A,@R1				; status register to A
			JB0 TXC2				; bit 0 TxRDY - buffer empty - proceed sending
			JMP TXC1				; wait more
TXC2		DEC R1					; 8251A data reg
			MOV A,R2				; move char to A
			MOVX @R1,A				; send it
			; restore registers
			ANL P2,#~IOM8155		; select 8155 RAM(deny UART, RIOT)
			MOV R1,#BACK_A-7		; backup RAM address
			MOVX A,@R1				; restore R0
			MOV R0,A				; move A to R0
			INC R1
			MOVX A,@R1				; restore R2
			MOV R2,A				; move A to R2
			INC R1
			MOVX A,@R1				; restore R3
			MOV R3,A
			INC R1
			MOVX A,@R1				; restore R4
			MOV R4,A
			INC R1
			MOVX A,@R1				; restore R5
			MOV R5,A
			INC R1
			MOVX A,@R1				; restore R6
			MOV R6,A
			INC R1
			MOVX A,@R1				; restore R7
			MOV R7,A				; move A to R7
			INC R1					; A backup address
			MOVX A,@R1				; restore A
			ORL P2,#IOM8155			; deselect 8155 RAM(allow UART, RIOT)
			RET
			;
			; logs char in A with registers preserved - R1 is changed though
LOGIMD		ANL P2,#~IOM8155		; select 8155 RAM(deny UART, RIOT)
			...
			MOV R2,A				; move to R2
	 		MOV R1,#CTRUART			; 8251A control/status reg
_LGI1		MOVX A,@R1				; status register to A
			JB0 _LGI2				; bit 0 TxRDY - buffer empty - proceed sending
			JMP _LGI1				; wait more
_LGI2		DEC R1					; 8251A data reg
			MOV A,R2				; move char to A
			MOVX @R1,A				; send it
			;
			; restore registers
			ANL P2,#~IOM8155		; select 8155 RAM(deny UART, RIOT)
			MOV R1,#BACK_A-1		; backup RAM address
			MOVX A,@R1				; restore R2
			MOV R2,A				; move A to R2
			INC R1
			MOVX A,@R1				; restore A
